...
from backend.base import QuantLinearBase
...
def get_quant_class(framework: str):
    ...
    if framework == 'torch':
        from mpttune.backend.torch.quantlinear import QuantLinear as QuantClass
    elif framework == 'cuda':
        from mpttune.backend.cuda.quantlinear import QuantLinear as QuantClass
    elif framework == 'triton':
        from mpttune.backend.triton.quantlinear import QuantLinear as QuantClass
    else:
...
def load_adapter(mpt, lora_apply_dir=None, lora_config=None, ddp=None):
    if lora_apply_dir is None:
        ...
        model = PeftModel.from_pretrained(
            mpt, lora_apply_dir, device_map=device_map,
